...
<div class="bg-white rounded-xl shadow-sm border overflow-hidden">
    <table class="w-full text-sm">
        ...
        <tbody class="divide-y">@forelse($employees as $employee)<tr><td class="px-5 py-3 font-medium">{{ $employee->full_name }}</td><td class="px-5 py-3">{{ $employee->position }}</td><td class="px-5 py-3 text-right">{{ number_format($employee->monthly_salary, 0) }}</td><td class="px-5 py-3 text-center"><x-status-badge :status="$employee->status" /></td><td class="px-5 py-3 text-right"><a href="{{ route('employees.show', $employee) }}" class="text-blue-600">Voir</a></td></tr>@empty<tr><td colspan="5" class="px-5 py-8 text-center text-slate-500">Aucun employé.</td></tr>@endforelse</tbody>
    </table>
